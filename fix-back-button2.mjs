...
const newButton = `function FloatingBackButton({ viewMode }: { viewMode: string }) {
  const navigate = useNavigate();
...
content = content.replace('{<FloatingBackButton />}', '{<FloatingBackButton viewMode={viewMode} />}');

// Ensure useLocation is imported
if (!content.includes('useLocation')) {
  content = content.replace('useNavigate, Link, Navigate } from', 'useNavigate, Link, Navigate, useLocation } from');
}
