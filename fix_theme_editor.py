...
theme_editor_regex = re.compile(r'function ThemeEditor\(\{ clients, setClients \}: \{ clients: Client\[\], setClients: any \}\) \{([\s\S]*?)\n\}\nfunction WatermarkEditor', re.MULTILINE)

match = theme_editor_regex.search(code)

if match:
  new_theme_editor = """function ThemeEditor({ clients, setClients }: { clients: Client[], setClients: any }) {
...
\nfunction WatermarkEditor"""
  ...
  new_code = theme_editor_regex.sub(new_theme_editor.replace('\\', '\\\\'), code)
  with open('src/pages/MasterRoom.tsx', 'w') as f:
      f.write(new_code)
  print('Replaced ThemeEditor successfully.')
else:
